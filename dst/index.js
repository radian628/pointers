(() => {
    ...
    return ViewPlugin.define((view) => {
      return {
        update(update) {
          ...
          decorations2 = RangeSet.of(highlights.map((hl) => {
            return Decoration.mark({
              class: defaultHighlightStyle.style([HL2Tag[hl.highlight]]) ?? void 0
  ...
  function pointersDiagnosticPlugin() {
    return linter((view) => {
      const diagnostics = parse(view.state.doc.toString()).errors.map((err) => {
        return {
          message: err.msg,
